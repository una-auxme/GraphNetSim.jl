...
# `update_meta!` writes one statistic type per call, so run both — the trial
# then selects between them via `norm_type` (:minmax / :meanstd).
update_meta!(ds_path, :minmax)
update_meta!(ds_path, :meanstd)
