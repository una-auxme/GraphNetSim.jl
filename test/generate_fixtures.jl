...
# ─────────────────────────────────────────────────────────────────────────────
# Check whether a dataset directory already has all required files
# ─────────────────────────────────────────────────────────────────────────────
...
# Shared HDF5 writer
# ─────────────────────────────────────────────────────────────────────────────
function _write_split(
    path, seeds, simulate_fn, n_particles, t_length, dims;
    has_types::Bool=false,
)
    h5open(path, "w") do f
        for (i, seed) in enumerate(seeds)
            result = simulate_fn(seed)
            pos = result[1]
            vel = result[2]
            acc = result[3]
            types = has_types ? result[4] : Int32.(ones(Int32, n_particles))

            g = create_group(f, "trajectory_$i")
            g["dt"] = result === nothing ? 0.0f0 : pos  # placeholder; overwritten below
            # Actually write properly:
            delete_object(g, "dt")

            g_real = g
            g_real["dt"] = dims == 3 ? 0.002f0 : 0.001f0  # set per-dataset below
            g_real["n_particles"] = Int64(n_particles)
            g_real["type"] = types
            for t in 1:t_length
                g_real["pos[$t]"] = pos[:, :, t]
                g_real["vel[$t]"] = vel[:, :, t]
