...
function train_step(strategy::BatchingStrategy, t::Tuple)
    ...
    shoot_loss, shoot_gs = Zygote.withgradient(
        ps -> train_loss(strategy, (prob, ps, u0, gt, mask, data["dt"], batches[b])),
        gns.train_state.parameters,
    )
...
function MultipleShooting(
    ...
    if isnothing(sense)
        # checkpointing=false for MultipleShooting: its short sub-interval solves can
        # produce single-point checkpoint segments, and the checkpointed reverse pass then
        # evaluates `abs(cpsol_t[end] - cpsol_t[end-1])` -> BoundsError at index 0
        # (SciMLSensitivity's interpolating_adjoint.jl). Storing the full forward
        # trajectory avoids that re-solve.
